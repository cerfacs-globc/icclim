...
def _get_valid_input(cfmeta: CfVarMetadata) -> list[str]:
    aliases = list(map(str.upper, cfmeta.aliases))
    aliases.append(cfmeta.standard_name.upper())
    aliases.append(cfmeta.long_name.upper())
    return aliases


CF_VAR_METADATA_REGISTRY = Registry[CfVarMetadata](
    catalog=[
        PR,
        TAS,
        TAS_MIN,
    ...
    lookup_method=_get_valid_input,
)
