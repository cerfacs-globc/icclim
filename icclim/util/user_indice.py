...
map_calc_params_required = {
    ...
    "nb_events": [
        "logical_operation",
        "thresh",
    ],  # 'link_logical_operations' ('and' or 'or' ) is required if multivariable indice
...
def check_features(var_name, in_files):
    # TODO see what is the point here, because a single file can bear multiple variable so it does not make sens to enforece nbr(varaible) == nbr(file)
        ...
        if type(in_files) is not list:
            ...
            if len(in_files) != len(var_name):
                raise MissingIcclimInputError(
                    "Number of input file lists must match number of input variables"
                )
...
def check_user_indice(indice_name, user_indice, time_range, var_name, out_unit):
    if user_indice is None:
        raise IOError(
            " 'user_indice' is required as a dictionary with user defined parameters."
        )
    else:
        ...
        if user_indice["calc_operation"] == "anomaly":
            ...
            if base_period_time_range is None:
                raise IOError(
                    'Time range of base period is required for anomaly-based user indices! Please, set the "base_period_time_range" parameter.'
                )
...
def get_key_by_value_from_dict_ui(my_map, my_value, inc, config_file):
    ...
    if icclim_indice:
        ...
        raise IOError(
            "'user_indice' or 'indice_name' are required to perform a calculation."
        )
...
def check_params(user_indice, time_range=None, vars=None):
    ...
    if (calc_op not in ["max", "min", "mean", "sum"]) and (
        set(required_params).intersection(given_params) != set(required_params)
    ):
        raise IOError("All theses parameters are required: {0}".format(required_params))

    if (
        calc_op in ["nb_events", "max_nb_consecutive_events"]
        and type(user_indice["thresh"]) == str
    ):
        if ("var_type") not in user_indice.keys():
            raise IOError(
                "If threshold value is a percentile,  'var_type' is required."
            )
    ...
    if type(vars) is list and len(vars) > 1:
        if calc_op in ["nb_events", "max_nb_consecutive_events"]:
            if (
                type(user_indice["logical_operation"]) is not list
                or type(user_indice["thresh"]) is not list
            ):
                raise IOError(
                    "If indice is based on {0} variables, then {0} 'logical_operations' and {0} 'thresh' "
                    "are required (each one must be a list with {0} elements).".format(
                        len(vars)
                    )
                )
            if (
                type(user_indice["logical_operation"]) is list
                and "link_logical_operations" not in user_indice.keys()
            ):
                raise IOError(
                    "If indice is based on {0} variables, then 'link_logical_operations' ('or' or 'and') is required.".format(
                        len(vars)
                    )
                )
        ...
        for i in range(len(vars)):
            if type(user_indice["thresh"][i]) is str:
                if ("var_type") not in user_indice.keys():
                    raise IOError(
                        "If at least one of thresholds is a percentile,  'var_type' "
...
def get_user_indice_params(user_indice, var_name, out_unit):
    ...
    if (
        ("date_event" in user_indice)
        and user_indice["calc_operation"] in ["mean", "sum"]
    ) or ("date_event" not in user_indice):
...
def get_user_indice(
    user_indice, arr, fill_val, vars, out_unit="days", dt_arr=None, pctl_thresh=None
):
    ...
    if indice_type not in [
        "user_indice_multivariable",
        ...
        for v in vars:
            ...
            if obj.calc_operation in ["min", "max", "mean", "sum"]:
                ...
                if obj.calc_operation == "nb_events":
                    ...
                    res = calc.get_max_nb_consecutive_days(
                        bin_arr,
                        logical_operation="e",
                        thresh=1,
                        coef=obj.coef,
        ...
        res = calc.get_nb_events_multivar(
            bin_arrs=binary_arrs,
            link_logical_operation=obj.link_logical_operation,
            fill_val=fill_val[
                vars[0]
            ],  ### fill_val is only to pass it to C function (for 'max_nb_consecutive_events')
            index_event=obj.date_event,
            out_unit=out_unit,
            max_consecutive=mc,
        )
