...
a = regrid.get_dst_src_files(file_list=[f1, f3, f4, f5, f2], varname=var, resolution_type=0) 
...
for f in files_src_grid:
    ...
    arr = regrid.get_regrided_var(f_src=f, f_dst=file_dst, varname=var)
    
    
    # Where we will safe the regridded file: for example, in the current directory, and the name of regridded file is the same as source file + "regridded_" before
    
    result_file_name = 'regridded_' + basename(f)

    regrid.write2netCDF_after_regridding(arr, f_src=f, f_dst=file_dst, f_out=result_file_name, var_src=var)
