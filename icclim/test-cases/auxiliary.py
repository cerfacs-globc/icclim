...
def get_callback(sect_dict):
    if "callback" in sect_dict.keys():
        function = sect_dict['callback'].split('.')[1]
        sect_dict['callback'] = getattr(callback, function)
...
def get_input_file_path(sect_dict, infolder):
    ...
    if isinstance(files2list[0], list):
        sect_dict['in_files'] = [[os.path.join(infolder, f) for f in fil] for fil in files2list]
    else:
...
def get_varnames_from_filenames(sect_dict):
    files = sect_dict['in_files']
    if isinstance(files[0], list):
        varnames = [os.path.basename(vname[0]).split("_")[0] for vname in files]
    else:
        varnames = os.path.basename(files[0]).split("_")[0]
    sect_dict['var_name'] = varnames
    return sect_dict
...
def make_string(non_string):
    ...
    elif type(non_string) is types.FunctionType:
    ...
    if type(non_string) is not types.StringType:
...
def get_test_md5hash(sect_dict, *args):
    ...
    hashid = hashlib.md5(hashid).hexdigest()
    return hashid
...
def get_suffix(in_files, dt_range, bt_range):
    ...
    if isinstance(in_files[0], list):
        suffix = re.sub("[0-9]{8}-[0-9]{8}.nc$", suffix + ".nc", "_".join(os.path.basename(in_files[0][0]).split("_")[1:]))
    else:
        suffix = re.sub("[0-9]{8}-[0-9]{8}.nc$", suffix + ".nc", "_".join(os.path.basename(in_files[0]).split("_")[1:]))
    return suffix
