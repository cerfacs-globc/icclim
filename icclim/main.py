...
from . import maps, time_subset, calc_ind, set_globattr, set_longname_units, \
    set_longname_units_custom_indices, calc_percentiles
from .icclim_exceptions import *
from .util import check, logging_info, util_nc, util_dt, files_order, arr_size, \
    callback, read, OCGIS_tile, calc
from .util import user_indice as ui

# Initial Config
global config_file
config_file = os.path.dirname(
    os.path.abspath(__file__)) + "/config_indice.json"
...
def indice(in_files,
        ...
        slice_mode='year',
        time_range=None,
        out_file=check.icclim_output_file_defaults('file_name'),
        # was: "./icclim_out.nc",
        threshold=None,
        N_lev=None,
        lev_dim_pos=1,
    ...
    lalalat = inc.variables["lat"].__array__()
    lololon = inc.variables["lon"].__array__()

    if indice_name is None:
        user_indice, indice_type = ui.check_user_indice(indice_name,
                                                        user_indice,
                                                        time_range, var_name,
                                                        out_unit)
    else:
        indice_type = ui.get_key_by_value_from_dict_ui(maps.map_indice_type,
                                                       indice_name, inc,
                                                       config_file)  # 'simple'/'multivariable'/'percentile_based'/'percentile_based_multivariable'

    if (indice_type in ['percentile_based',
                        'percentile_based_multivariable'] or indice_type.startswith(
        'user_indice_percentile_')) and base_period_time_range is None:
        raise IOError(
            'Time range of base period is required for percentile-based indices! Please, set the "base_period_time_range" parameter.')
    ...
    indice_dim = util_nc.copy_var_dim(inc, onc, var_name[0],
                                      lev_dim_pos=lev_dim_pos)  # tuple ('time', 'lat', 'lon')
    ...
    ind = onc.createVariable(indice_name, ind_type, indice_dim,
                             fill_value=fill_val)
    ...
    if indice_type.startswith('user_indice_') and user_indice[
        'date_event'] == True:
        if user_indice['calc_operation'] in ['min', 'max']:
            date_event = onc.createVariable('date_event', 'f', indice_dim,
                                            fill_value=fill_val)
        ...
        elif user_indice['calc_operation'] in ['nb_events',
                                               'max_nb_consecutive_events',
                                               'run_mean', 'run_sum']:
            date_event_start = onc.createVariable('date_event_start', 'f',
                                                  indice_dim,
                                                  fill_value=fill_val)
            # we set the same 'calendar' and 'units' attributes as those of netCDF var 'time'
            ...
            date_event_end = onc.createVariable('date_event_end', 'f',
                                                indice_dim,
                                                fill_value=fill_val)
            # we set the same 'calendar' and 'units' attributes as those of netCDF var 'time'
    ...
    time_range = util_dt.get_time_range(files=VARS_in_files[var_name[0]],
                                        ...
                                        temporal_var_name=indice_dim[0])

    if base_period_time_range is not None:
        base_period_time_range = util_dt.harmonize_hourly_timestamp(
            base_period_time_range, ncVar_time.calendar, time_range[0])
    ...
    for v in var_name:
        ...
        dict_files_years_to_process = files_order.get_dict_files_years_to_process_in_correct_order(
            files_list=VARS_in_files[v], time_range=time_range)
        ...
        if indice_type in ["percentile_based",
                           "percentile_based_multivariable"] or indice_type.startswith(
            'user_indice_percentile_') or (
                indice_type.startswith('user_indice_') and user_indice[
            'calc_operation'] == 'anomaly'):
            dict_files_years_to_process_base = files_order.get_dict_files_years_to_process_in_correct_order(
                files_list=VARS_in_files[v], time_range=base_period_time_range)
            VARS[v]['files_years_base'] = dict_files_years_to_process_base

        dim_name = util_nc.check_unlimited(VARS_in_files[v][0])
        tile_dimension = arr_size.get_tile_dimension(
            in_files=list(dict_files_years_to_process.keys()),
            var_name=v,
    ...
    # chunk tiles
    tile_map = OCGIS_tile.get_tile_schema(nrow=var_shape1, ncol=var_shape2,
                                          tdim=tile_dimension, origin=0)
    ...
    for tile_id in tile_map:
        ...
        if len(tile_map) > 1:
            logging.info("Loading data: chunk " + str(
                int(chunk_counter + 1)) + '/' + str(len(tile_map)) + " ...")
        else:
        ...
        for v in var_name:
            if chunk_counter == 0:
                ...
                ncVar_time = inc.variables[
                    dimensions_list_current_var[index_time]]
            ...
            nc = MFDataset(filename, 'r',
                           aggdim=dim_name)  # VARS[v]['files_years'].keys(): files of current variable
            ...
            arrs_current_chunk = util_nc.get_values_arr_and_dt_arr(
                ncVar_temporal=var_time, ncVar_values=var,
            ...
            if indice_type.startswith('user_indice_') and user_indice[
                ...
                var = ncb.variables[v]
                arrs_current_chunk_ref = util_nc.get_values_arr_and_dt_arr(
                    ncVar_temporal=var_time, ncVar_values=var,
                    fill_val=VARS[v]['fill_value'],
                    time_range=base_period_time_range,
                    N_lev=N_lev,
                ...
                VARS[v]['values_arr_ref'] = arrs_current_chunk_ref[
                    1]  # arrs_current_chunk_ref[1]: values, arrs_current_chunk_ref[0]: dt_arr

            if indice_type in ["percentile_based",
                               "percentile_based_multivariable"] or indice_type.startswith(
                'user_indice_percentile_'):
                MF_nc = [MF_nc for MF_nc in VARS[v]['files_years_base'].keys()]
                ncb = MFDataset(MF_nc, 'r', aggdim=dim_name)
                var_time = ncb.variables[indice_dim[0]]
                var = ncb.variables[v]
                arrs_base_current_chunk = util_nc.get_values_arr_and_dt_arr(
                    ncVar_temporal=var_time, ncVar_values=var,
                    fill_val=VARS[v]['fill_value'],
                    time_range=base_period_time_range,
                    N_lev=N_lev,
            ...
            dict_temporal_slices = time_subset.get_dict_temporal_slices(
                dt_arr=VARS[v]['dt_arr'],
        ...
        if nb_user_thresholds == 0:
            ...
            if chunk_counter == 0:
                indice_arr = numpy.ma.zeros(
                    (len(dict_temporal_slices), var_shape1, var_shape2),
                    dtype=ind_type)
            indice_tuple_current_chunk = get_indice_from_dict_temporal_slices(
                ...
                callback=callback,
                callback_percentage_total=callback_percentage_total,
            ...
            if indice_type.startswith('user_indice_'):
                if user_indice['date_event'] == True:
                    if user_indice['calc_operation'] in ['min', 'max']:
                        date_event_arr_current_chunk = \
                            indice_tuple_current_chunk[3]

                    elif user_indice['calc_operation'] in ['nb_events',
                                                           'max_nb_consecutive_events',
                                                           'run_mean',
                                                           'run_sum']:
                        date_event_start_arr_current_chunk = \
                            indice_tuple_current_chunk[3]
                        date_event_end_arr_current_chunk = \
                            indice_tuple_current_chunk[4]

            indice_arr[:, i1_row_current_tile:i2_row_current_tile,
            i1_col_current_tile:i2_col_current_tile] = indice_arr_current_chunk
            acc.append(indice_tuple_current_chunk[-1])
            ...
            for t in user_thresholds:
                if chunk_counter == 0:
                    ...
                    dict_threshold_indice_arr[t] = numpy.zeros(
                        (len(dict_temporal_slices), var_shape1, var_shape2),
                        dtype=ind_type)
    ...
    if nb_user_thresholds == 0:
        ...
        for t, key in zip(range(nb_user_thresholds),
                          dict_threshold_indice_arr.keys()):
    ...
    #### we set global attributes for standard indices (not for user defined indices)
        ...
        if threshold != None:
            onc.setncattr('title',
                          'Index {0} with user defined threshold'.format(
                              indice_name))
        else:
        ...
        if threshold == None:
            ...
            eval(
                'set_longname_units_custom_indices.' + indice_name + '_setvarattr(ind, threshold)')
            ...
            if nb_user_thresholds > 1:
                eval(
                    'set_longname_units_custom_indices.' + indice_name + '_setthresholdattr(thresholdvar)')
...
def get_indice_from_dict_temporal_slices(indice_name,
    ...
    vars_dict_keys_temp_0 = \
        [vars_dict_keys for vars_dict_keys in vars_dict.keys()][0]
    ...
    if user_indice == None:  ### standard indices
        indice_type = get_key_by_value_from_dict(maps.map_indice_type,
                                                 indice_name)
    else:  ### user indices
    ...
    for v in vars_dict.keys():
        if 'var_type' in vars_dict[v].keys():  # no matter if it is 'p' or 't'
            ...
            intersecting_years = list(
                set(years_base).intersection(years_study))
    ...
    if callback != None:
        ...
        if thresh == None:
            percentage_slice = (callback_percentage_total * 1.0) / (
                    nb_t_slices * nb_chunks)
        elif thresh != None:
            percentage_slice = (callback_percentage_total * 1.0) / (
                    nb_t_slices * nb_user_thresholds * nb_chunks)
    ...
    for slice_ in t_slices:  # for each temporal slice
        ...
        dt_arr_ = vars_dict_keys_0['temporal_slices'][slice_][
            2]  ###   vars_dict.keys()[0] is the first target variable in the dictionary
        ...
        if indice_type == 'simple':
            ...
            vars_dict_keys_temp_1 = \
                [vars_dict_keys for vars_dict_keys in vars_dict.keys()][1]
            ...
            if user_indice['date_event'] == True:
                ...
                if user_indice['calc_operation'] in ['min', 'max']:
                    ...
                    date_event_slice = calc.get_date_event_arr(dt_arr=dt_arr_,
                                                               index_arr=indice_slice_date_event,
                                                               time_calendar=t_calend,
                                                               time_units=t_units,
                                                               fill_val=fill_val)
                    date_event_slice = date_event_slice.reshape(-1,
                                                                date_event_slice.shape[
                                                                    0],
                                                                date_event_slice.shape[
                                                                    1])  #  2D --> 3D
                    ...
                    indice_slice_date_event_start = \
                        indice_slice_date_event_bounds[0]
                    indice_slice_date_event_end = \
                        indice_slice_date_event_bounds[1]

                    date_event_slice_start = calc.get_date_event_arr(
                        dt_arr=dt_arr_,
                        index_arr=indice_slice_date_event_start,
                        time_calendar=t_calend, time_units=t_units,
                        fill_val=fill_val)

                    date_event_slice_end = calc.get_date_event_arr(
                        dt_arr=dt_arr_, index_arr=indice_slice_date_event_end,
                        time_calendar=t_calend, time_units=t_units,
                        fill_val=fill_val)

                    date_event_slice_start = date_event_slice_start.reshape(-1,
                                                                            date_event_slice_start.shape[
                                                                                0],
                                                                            date_event_slice_start.shape[
                                                                                1])  #  2D --> 3D
                    date_event_slice_end = date_event_slice_end.reshape(-1,
                                                                        date_event_slice_end.shape[
                                                                            0],
                                                                        date_event_slice_end.shape[
                                                                            1])  #  2D --> 3D
                ...
                if user_indice['calc_operation'] == 'anomaly':
                    ...
                    dic_args = {'user_indice': user_indice,
                                'arr':         [values_arr, values_arr_ref],
                                'fill_val':    fill_val, 'vars': vars_dict.keys(),
                                'out_unit':    out_unit}
            ...
            if user_indice['date_event'] == True:
                ...
                indice_slice_date_event_bounds = indice_[1]
                indice_slice_date_event_start = indice_slice_date_event_bounds[
                    0]
                indice_slice_date_event_end = indice_slice_date_event_bounds[1]

                date_event_slice_start = calc.get_date_event_arr(
                    dt_arr=dt_arr_, index_arr=indice_slice_date_event_start,
                    time_calendar=t_calend, time_units=t_units,
                    fill_val=fill_val_)

                date_event_slice_end = calc.get_date_event_arr(dt_arr=dt_arr_,
                                                               index_arr=indice_slice_date_event_end,
                                                               time_calendar=t_calend,
                                                               time_units=t_units,
                                                               fill_val=fill_val_)
            ...
            indice_slice_i = numpy.ma.zeros(
                (len(reduced_base_years_list), nb_rows, nb_columns))
            ...
            for ytd in reduced_base_years_list:  ### ytd: year to duplicate
                ...
                for v in vars_dict.keys():
                    ...
                    if 'var_type' in vars_dict[
                        v].keys():  # if percentile-based index, we will compute a pctl threshold

                        ### we want to get the percentile value:
                        if indice_type == 'percentile_based':
                            pctl_value = \
                                maps.map_indice_percentile_value[indice_name][
                                    0]  ### for standard indices, we take it from "maps.map_indice_percentile_value"
                        else:
                            pctl_value = int((user_indice[v]['thresh'])[
                                             1:])  ### for user index, we take it from "user_indice" dictionary

                        if vars_dict[v]['var_type'] == 'p':
                            # precipitations
                            # we compute pctl array only one time
                            if slice_counter == 0 and ytd_counter == 0:
                                pctl_arr = calc_percentiles.get_percentile_arr(
                                    arr=vars_dict[v]['base']['values_arr'],
                            ...
                            if current_intersecting_year != -9999 or cnt == 1:
                                new_arrs_base = time_subset.get_resampled_arrs(
                                    dt_arr=vars_dict[v]['base']['dt_arr'],
                                    values_arr=vars_dict[v]['base'][
                                        'values_arr'],
                                ...
                                if current_intersecting_year == -9999:
                                    logging.info(
                                        "Daily Percentiles calculation for out-of-base years. Please be patient...")
                                    bootstrapping = False
                                else:
                                    logging.info(
                                        "[Bootstrapping] Daily Percentiles calculation for in-base year %d for %s. Duplicating %s. Please be patient...",
                                        current_intersecting_year, str(v),
                                        str(ytd))
                                    bootstrapping = True

                                daily_pctl_dict = calc_percentiles.get_percentile_dict(
                                    arr=new_arrs_base[1],
                ...
                if all(test_v) == 1:  ### if "test_v" contains only 1, i.e. we checked all variables
                    ...
                    if indice_type in ['percentile_based',
                        ...
                        if indice_name in ['R75p', 'R75pTOT', 'R95p',
                            ...
                            if ytd_counter == 0:
                                ...
                                numpy.ma.set_fill_value(indice_slice_i,
                                                        vars_dict[va][
                                                            'fill_value'])
                                indice_slice_i[:, :, :] = calc_ind.zzz(
                                    indice_name,
                                    **dic_args)  # 3D with the same  2D arrays
                        ...
                        elif indice_name in ['TG10p', 'TX10p', 'TN10p',
                                             'TG90p', 'TX90p', 'TN90p', 'WSDI',
                                             'CSDI']:
                            va = [vars_dict_keys_0 for vars_dict_keys_0 in
                                  vars_dict.keys()][0]
                            if current_intersecting_year == -9999:
                            ...
                            numpy.ma.set_fill_value(indice_slice_i,
                                                    vars_dict[va][
                                                        'fill_value'])
                            indice_slice_i[ytd_counter, :, :] = calc_ind.zzz(
                                indice_name, **dic_args)  # 3D
                            ...
                            if vars_dict[va]['var_type'] == 'p':
                                ...
                                values = calc.get_wet_days(arr=vars_dict[va][
                                    'temporal_slices'][slice_][3], fill_val=
                                                           vars_dict[va][
                                                               'fill_value'])
                                pt = pctl_arr
                            else:  ### vars_dict[va]['var_type']=='t'
                                values = arr = \
                                    vars_dict[va]['temporal_slices'][slice_][3]
                            ...
                            if user_indice['date_event'] == True:
                                ...
                                numpy.ma.set_fill_value(indice_slice_i,
                                                        vars_dict[va][
                                                            'fill_value'])
                                indice_slice_i[ytd_counter, :, :] = \
                                    indice_slice_[0]

                                if ytd_counter == 0:

                                    indice_slice_date_event_bounds = \
                                        indice_slice_[1]
                    ...
                    # if save_percentile:
                    #     percentile_array = util_dt.from_OrderedDict_to_array(pd,
                    #                                                          dt_arr_,
                    #                                                          indice_slice)
                    #     util_nc.save_percentile_netcdf(out_file, percentile_array)
                        ...
                        for v in vars_dict.keys():
                            ...
                            if 'var_type' in vars_dict[v].keys():
                                ...
                                if vars_dict[v]['var_type'] == 'p':
                                    ...
                                    arrs[v] = \
                                        vars_dict[v]['temporal_slices'][slice_][3]
                        ...
                        if user_indice['date_event'] == True:
                            ...
                            if ytd_counter == 0:
                                indice_slice_date_event_bounds = indice_slice_[
                                    1]
                                ...
                                if current_intersecting_year == -9999:  ### for out-of-base years: ok
                                    ...
                                    ### ==> we fill all indices by -1 to get fill_value in output date of event array for in-base years
                                    indice_slice_date_event_start = \
                                        indice_slice_date_event_bounds[0] * 0 - 1
                                    indice_slice_date_event_end = \
                                        indice_slice_date_event_bounds[1] * 0 - 1
            ...
            if indice_type in ['user_indice_percentile_based',
                               'user_indice_percentile_based_multivariable'] and \
                    user_indice['date_event'] == True:
                date_event_slice_start = calc.get_date_event_arr(
                    dt_arr=dt_arr_, index_arr=indice_slice_date_event_start,
                    time_calendar=t_calend, time_units=t_units,
                    fill_val=vars_dict_keys_0['fill_value'])
            ...
            if out_unit == 'days':
                indice_slice = numpy.ma.around(
                    indice_slice)  ### we need them to be integer for out_unit="days"
        #             else:
        ...
        indice_slice = indice_slice.reshape(-1, indice_slice.shape[0],
                                            indice_slice.shape[
                                                1])  #  2D --> 3D
        ...
        if slice_counter == 0:
            ...
            indice_arr = numpy.ma.concatenate((indice_arr, indice_slice),
                                              axis=0)
        ...
        if indice_type.startswith('user_indice_') and user_indice[
            ...
            if slice_counter == 0:
                ...
                if user_indice['calc_operation'] in ['min', 'max']:
                    date_event_arr = numpy.concatenate(
                        (date_event_arr, date_event_slice), axis=0)
                else:
                    date_event_start_arr = numpy.concatenate(
                        (date_event_start_arr, date_event_slice_start), axis=0)
                    date_event_end_arr = numpy.concatenate(
                        (date_event_end_arr, date_event_slice_end), axis=0)
    ...
    if save_percentile:
        if "precip" in pd.keys():
            ...
            percentile_array = util_dt.from_OrderedDict_to_array(pd,
                                                                 dt_arr_,
                                                                 indice_slice)
            util_nc.save_percentile_netcdf(out_file, lalalat, lololon, percentile_array)
    if indice_type.startswith('user_indice_') and user_indice[
        'date_event'] == True:
        if user_indice['calc_operation'] in ['min', 'max']:
            ...
            return (
                dt_centroid_arr, dt_bounds_arr, indice_arr, date_event_start_arr,
                date_event_end_arr, pctl_arr)
    else:

        return (dt_centroid_arr, dt_bounds_arr, indice_arr, pctl_arr)
