...
def indice(
    ...
    if indice_name is None:
        user_indice, indice_type = ui.check_user_indice(
            indice_name, user_indice, time_range, var_name, out_unit
        )
    else:
        indice_type = ui.get_key_by_value_from_dict_ui(
            maps.map_indice_type, indice_name, inc, config_file
        )  # 'simple'/'multivariable'/'percentile_based'/'percentile_based_multivariable'

    if (
        indice_type in ["percentile_based", "percentile_based_multivariable"]
        or indice_type.startswith("user_indice_percentile_")
    ) and base_period_time_range is None:
        raise IOError(
    ...
    indice_dim = util_nc.copy_var_dim(
        inc, onc, var_name[0], lev_dim_pos=lev_dim_pos
    )  # tuple ('time', 'lat', 'lon')
    ...
    if indice_type.startswith("user_indice_") and user_indice["date_event"] == True:
        if user_indice["calc_operation"] in ["min", "max"]:
            date_event = onc.createVariable(
                "date_event", "f", indice_dim, fill_value=fill_val
            )
        ...
        elif user_indice["calc_operation"] in [
            "nb_events",
            "max_nb_consecutive_events",
            "run_mean",
            "run_sum",
        ]:
            date_event_start = onc.createVariable(
                "date_event_start", "f", indice_dim, fill_value=fill_val
            )
            # we set the same 'calendar' and 'units' attributes as those of netCDF var 'time'
            date_event_start.__setattr__("calendar", ncVar_time.calendar)
            date_event_start.__setattr__("units", ncVar_time.units)

            date_event_end = onc.createVariable(
                "date_event_end", "f", indice_dim, fill_value=fill_val
            )
            # we set the same 'calendar' and 'units' attributes as those of netCDF var 'time'
            date_event_end.__setattr__("calendar", ncVar_time.calendar)
            date_event_end.__setattr__("units", ncVar_time.units)

    time_range = util_dt.get_time_range(
        files=VARS_in_files[var_name[0]],
    ...
    if (
        indice_type.startswith("user_indice_")
        and user_indice["calc_operation"] == "anomaly"
    ):
        time_range2 = util_dt.get_time_range(
            files=VARS_in_files[var_name[0]],
    ...
    # chunk tiles
    tile_map = OCGIS_tile.get_tile_schema(
        nrow=var_shape1, ncol=var_shape2, tdim=tile_dimension, origin=0
    )
    ...
    for tile_id in tile_map:
        ...
        for v in var_name:
            ...
            nc = MFDataset(
                filename, "r", aggdim=dim_name
            )  # VARS[v]['files_years'].keys(): files of current variable
            ...
            if (
                ...
                VARS[v]["values_arr_ref"] = arrs_current_chunk_ref[
                    1
                ]  # arrs_current_chunk_ref[1]: values, arrs_current_chunk_ref[0]: dt_arr

            if indice_type in [
                "percentile_based",
                "percentile_based_multivariable",
            ] or indice_type.startswith("user_indice_percentile_"):
                MF_nc = [MF_nc for MF_nc in VARS[v]["files_years_base"].keys()]
                ncb = MFDataset(MF_nc, "r", aggdim=dim_name)
                var_time = ncb.variables[indice_dim[0]]
        ...
        if nb_user_thresholds == 0:
            ...
            if chunk_counter == 0:
                indice_arr = numpy.ma.zeros(
                    (len(dict_temporal_slices), var_shape1, var_shape2), dtype=ind_type
                )
            ...
            )  ## tuple: (dt_centroid_arr, dt_bounds_arr, indice_arr)
            ...
            if indice_type.startswith("user_indice_"):
                if user_indice["date_event"] == True:
                    if user_indice["calc_operation"] in ["min", "max"]:
                        ...
                        date_event_start_arr_current_chunk = indice_tuple_current_chunk[
                            3
                        ]
            ...
            indice_arr[
                ...
                i1_col_current_tile:i2_col_current_tile,
            ] = indice_arr_current_chunk
    ...
    #### we set global attributes for standard indices (not for user defined indices)
        ...
        if threshold != None:
            onc.setncattr(
                "title", "Index {0} with user defined threshold".format(indice_name)
            )
        else:
        ...
        set_globattr.institution(
            onc, institution_str="Climate impact portal (http://climate4impact.eu)"
        )
        ...
        if threshold == None:
            ...
            if nb_user_thresholds > 1:
                eval(
                    "set_longname_units_custom_indices."
                    + indice_name
                    + "_setthresholdattr(thresholdvar)"
                )
...
def get_indice_from_dict_temporal_slices(
    indice_name,
    vars_dict,
    thresh=None,
    window_width=None,
    ...
    pctl_calc_method = (
        {}
    )  ### dictionary to separate pctl thresholds: computed with bootstrapping (for in-base years) or without bootstrapping (for out-of-base years)
    ...
    for slice_ in t_slices:  # for each temporal slice
        ...
        dt_arr_ = vars_dict_keys_0["temporal_slices"][slice_][
            2
        ]  ###   vars_dict.keys()[0] is the first target variable in the dictionary
        ...
        if indice_type == "simple":
            ...
            vars_dict_keys_temp_1 = [
                vars_dict_keys for vars_dict_keys in vars_dict.keys()
            ][1]
            ...
            dic_args = {
                "arr1": values_arr_tasmax,
                "arr2": values_arr_tasmin,
                "fill_val1": fill_val,
                "fill_val2": fill_val2,
            }
            ...
            if user_indice["date_event"] == True:
                ...
                if user_indice["calc_operation"] in ["min", "max"]:
                    ...
                    date_event_slice = calc.get_date_event_arr(
                        dt_arr=dt_arr_,
                    ...
                    date_event_slice = date_event_slice.reshape(
                        -1, date_event_slice.shape[0], date_event_slice.shape[1]
                    )  #  2D --> 3D
                    ...
                    date_event_slice_start = date_event_slice_start.reshape(
                        -1,
                        date_event_slice_start.shape[0],
                        date_event_slice_start.shape[1],
                    )  #  2D --> 3D
                    date_event_slice_end = date_event_slice_end.reshape(
                        -1, date_event_slice_end.shape[0], date_event_slice_end.shape[1]
                    )  #  2D --> 3D
            ...
            if user_indice["date_event"] == True:
                ...
                date_event_slice_start = date_event_slice_start.reshape(
                    -1, date_event_slice_start.shape[0], date_event_slice_start.shape[1]
                )  #  2D --> 3D
                date_event_slice_end = date_event_slice_end.reshape(
                    -1, date_event_slice_end.shape[0], date_event_slice_end.shape[1]
                )  #  2D --> 3D
            ...
            for ytd in reduced_base_years_list:  ### ytd: year to duplicate
                ...
                for v in vars_dict.keys():
                    ...
                    if (
                        "var_type" in vars_dict[v].keys()
                    ):  # if percentile-based index, we will compute a pctl threshold

                        ### we want to get the percentile value:
                        if indice_type == "percentile_based":
                            pctl_value = maps.map_indice_percentile_value[indice_name][
                                0
                            ]  ### for standard indices, we take it from "maps.map_indice_percentile_value"
                        else:
                            pctl_value = int(
                                (user_indice[v]["thresh"])[1:]
                            )  ### for user index, we take it from "user_indice" dictionary
                ...
                if (
                    all(test_v) == 1
                ):  ### if "test_v" contains only 1, i.e. we checked all variables

                    if indice_type in [
                        "percentile_based",
                        "user_indice_percentile_based",
                        ...
                        if indice_name in [
                            ...
                            if ytd_counter == 0:
                                va = [
                                    vars_dict_keys_0
                                    for vars_dict_keys_0 in vars_dict.keys()
                                ][0]
                                dic_args = {
                                    "arr": vars_dict[va]["temporal_slices"][slice_][3],
                                ...
                                numpy.ma.set_fill_value(
                                    indice_slice_i, vars_dict[va]["fill_value"]
                                )
                                indice_slice_i[:, :, :] = calc_ind.zzz(
                                    indice_name, **dic_args
                                )  # 3D with the same  2D arrays
                            ...
                            if vars_dict[va]["var_type"] == "p":
                                ...
                                values = calc.get_wet_days(
                                    arr=vars_dict[va]["temporal_slices"][slice_][3],
                                    fill_val=vars_dict[va]["fill_value"],
                                )
                                pt = pctl_arr
                            else:  ### vars_dict[va]['var_type']=='t'
                                values = arr = vars_dict[va]["temporal_slices"][slice_][
                                    3
                                ]
                            ...
                            if user_indice["date_event"] == True:
                                ...
                                if ytd_counter == 0:
                                    ...
                                    if vars_dict[va]["var_type"] == "p":
                                        ...
                                        indice_slice_date_event_end = indice_slice_date_event_bounds[
                                            1
                                        ]
                                        ...
                                        if (
                                            ...
                                            indice_slice_date_event_start = (
                                                indice_slice_date_event_bounds[0] * 0
                                                - 1
                                            )
                                            indice_slice_date_event_end = (
                                                indice_slice_date_event_bounds[1] * 0
                                                - 1
                                            )
                        ...
                        for v in vars_dict.keys():
                            ...
                            if "var_type" in vars_dict[v].keys():
                                ...
                                if vars_dict[v]["var_type"] == "p":
                                    ...
                                    # we need to process only "wet" days (RR >= 1.0 mm)
                                    arrs[v] = calc.get_wet_days(
                                        arr=vars_dict[v]["temporal_slices"][slice_][3],
                                        fill_val=vars_dict[v]["fill_value"],
                                    )
            ...
            if (
                ...
                in [
                    "user_indice_percentile_based",
                    "user_indice_percentile_based_multivariable",
                ]
                and user_indice["date_event"] == True
            ):
                date_event_slice_start = calc.get_date_event_arr(
                    dt_arr=dt_arr_,
                    index_arr=indice_slice_date_event_start,
            ...
            if out_unit == "days":
                indice_slice = numpy.ma.around(
                    indice_slice
                )  ### we need them to be integer for out_unit="days"
        #             else:
        ...
        indice_slice = indice_slice.reshape(
            -1, indice_slice.shape[0], indice_slice.shape[1]
        )  #  2D --> 3D
        ...
        if callback != None:
            ...
            if (
                ...
                or indice_type == "percentile_based_multivariable"
            ):
                if current_intersecting_year == -9999:
